      ...
      style={
        {
          ...
          "--width": `${width}`,
          "--aspect-ratio": `${ar}`,
          "--background": `linear-gradient(hsl(180 100% 50%), transparent)`,
...
export const WarpBackground: React.FC<WarpBackgroundProps> = ({
  ...
  const topBeams = useMemo(() => (isMounted ? generateBeams() : []), [isMounted, generateBeams])
  const rightBeams = useMemo(() => (isMounted ? generateBeams() : []), [isMounted, generateBeams])
  const bottomBeams = useMemo(() => (isMounted ? generateBeams() : []), [isMounted, generateBeams])
  const leftBeams = useMemo(() => (isMounted ? generateBeams() : []), [isMounted, generateBeams])
