...
function getDropdownPosition({
  ...
  if (preferSide === "top") {
    openBelow = spaceAbove >= panelHeight ? false : spaceBelow > spaceAbove;
  } else if (preferSide === "bottom") {
    openBelow = spaceBelow >= panelHeight ? true : spaceBelow >= spaceAbove;
  } else {
    openBelow =
  ...
  return (
    <>
      <button
        ...
        className="mr-9 ml-auto flex items-center justify-center rounded-full py-1 text-muted-foreground/30 cursor-not-allowed"
        disabled
        ref={triggerRef}
        type="button"
      >
      ...
      <AnimatePresence>
        {expanded && (
          <motion.button
            ...
            aria-label={hasValue ? "Send prompt" : "Voice input disabled"}
            className={`absolute right-2 bottom-2 flex size-8 items-center justify-center transition-colors ${
              hasValue 
                ? "bg-accent text-accent-foreground hover:opacity-90 cursor-pointer" 
                : "bg-transparent text-muted-foreground/30 cursor-default pointer-events-none"
            }`}
