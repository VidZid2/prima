...
const ToastContainer = () => {
  ...
  const getFinalTransform = (index: number, length: number) => {
    ...
    if (isMobile) {
      return `translate3d(0, calc(${translateY}px - 100%), ${z}px) scale(${scale})`;
    }
  ...
  return (
    <div
      className="fixed top-20 left-4 right-4 md:top-auto md:left-auto md:bottom-4 md:right-4 z-[9999] pointer-events-none md:w-[420px]"
      style={{ height: isMobile ? undefined : containerHeight }}
    >
      <div
        ...
        {toasts.map((toast, index) => {
          ...
          return (
            <div
              ...
              className={clsx(
                "absolute right-0 rounded-xl leading-[21px] p-4 h-fit w-full md:w-[420px]",
                isMobile ? "top-0 left-0" : "bottom-0",
                "border border-white/10 shadow-[inset_0_1px_1px_rgba(255,255,255,0.15),_0_4px_16px_rgba(0,0,0,0.4)]",
                {
                  message: "bg-gradient-to-b from-[#2a2a2f] to-[#121214] text-zinc-100",
                ...
                (isVisible && !toast.isClosing) ? "opacity-100" : "opacity-0",
                (index < lastVisibleStart || toast.isClosing) && "pointer-events-none"
              )}
              style={{
                transition: "all .35s cubic-bezier(.25,.75,.6,.98)",
                transform: (shownIds.includes(toast.id) && !toast.isClosing)
                  ? getFinalTransform(index, toasts.length)
                  : (isMobile ? "translate3d(0, -100%, 150px) scale(1)" : "translate3d(0, 100%, 150px) scale(1)")
              }}
            >
                ...
                <div className="w-full h-full flex items-center justify-between gap-4">
                  ...
                  {!toast.action && (
                    <div className="flex gap-1">
                      ...
                      >
                        <CloseIcon className={{
                          message: "fill-zinc-400 hover:fill-zinc-100 transition-colors",
