    ...
    <div className={cn("mx-auto flex items-center justify-center", className)}>
      <AnimatePresence>
        ...
        >
          ...
          >
            <SettingsFilled className="text-cyan-400 dark:text-cyan-300" />
          </motion.div>
          <motion.div
            initial="hidden"
            animate={isHovered ? "visible" : "hidden"}
          ...
          <div className="relative flex h-[35px] items-center gap-1 rounded-full border border-white/10 bg-gradient-to-b from-[#2a2a2f] to-[#121214] shadow-[inset_0_1px_1px_rgba(255,255,255,0.15),_0_4px_16px_rgba(0,0,0,0.4)] pl-3 pr-2 text-sm backdrop-blur-md">
            {href ? (
              <Link
                href={href}
                className="focus-visible:shadow-focus-ring rounded-xs my-[-1px] cursor-pointer border-none bg-transparent px-0 py-1 font-sans text-[13px] font-medium text-cyan-200 outline-none hover:text-cyan-50 focus-visible:!shadow-cyan-500 dark:text-cyan-100 dark:hover:text-cyan-50"
            ...
            <span className="text-[0.8125rem] text-cyan-300/80 dark:text-cyan-200/80">
              {description}
            </span>
            {onClose && (
              <button 
                ...
                className="m-0 flex h-6 !w-6 shrink-0 cursor-pointer items-center justify-center rounded-[4px] border-0 bg-transparent p-0 text-cyan-400 hover:bg-cyan-500/20 dark:hover:bg-cyan-500/20"
              >
                <X size={16} className="text-cyan-400 dark:text-cyan-300" />
              </button>
            )}
          </div>
        </motion.div>
