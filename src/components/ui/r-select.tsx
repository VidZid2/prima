...
const selectItemClassName =
  "group relative flex min-h-11 cursor-pointer touch-manipulation select-none items-center gap-3 rounded-lg py-2.5 pr-8 pl-3 text-[color:var(--sel-foreground)] text-sm outline-none transition-colors";

const selectItemHighlightClassName =
  "absolute inset-0 -z-10 rounded-lg bg-black shadow-[inset_0_1px_0_rgba(255,255,255,0.03),0_1px_3px_rgba(0,0,0,0.5)] border border-zinc-800/50";
...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger {...props} asChild>
      <button className={cn(selectThemeClassName, selectTriggerClassName, className)} data-size={size} data-slot="select-trigger" type="button">
        {children}
        <SelectPrimitive.Icon asChild>
          <motion.span animate={{ rotate: open ? 180 : 0 }} className="shrink-0" transition={chevronTransition}>
...
function SelectItem({
  ...
  const labelContent = <span>{children}</span>;
  const textContent = (
    <div className="flex flex-col min-w-0 justify-center">
      <span className="truncate leading-snug">{children}</span>
      {description && (
        <span className="text-[11px] leading-tight text-zinc-400 truncate max-w-full mt-0.5">
          {description}
        </span>
      )}
    </div>
  ...
  registerItemLabel(value, labelContent);
    ...
    >
      ...
      >
        ...
        <SelectPrimitive.ItemText asChild>
          <div className="relative z-10 flex min-w-0 flex-1 flex-col text-left [&_svg]:shrink-0">
            {itemContent}
          </div>
